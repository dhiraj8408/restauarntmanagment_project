...
float isAvailable(restaurant rst[], char items[][100], char usr_city[], int x, int no_of_items,int quantity[])
{
    int no_of_generic = 0;
    int no_of_special = 0;
    int i = 0;
    ...
    if (strcmp(rst[x].rst_address.city_name, usr_city) == 0)
    {
        ...
        for (int i = 0; i < no_of_items; i++)
        {
            ...
            while (low <= high && found == 0)
            {
                ...
                {
                    found = 1;
                    delivery_time = delivery_time + rst[x].generic.service_time[mid] * quantity[i];
                }
            ...
            if (found == 0)
            {
                ...
                while (low <= high && found == 0)
                {
                    ...
                    {
                        found = 1;
                        delivery_time = delivery_time + rst[x].special.service_time[mid] * quantity[i];
                    }
            ...
            if (found == 0)
            {
                for (int j = 0; j < NO_OF_DAYS && found == 0; j++)
                {
                    ...
                    {
                        found = 1;
                        delivery_time = delivery_time + rst[x].item_of_the_day.service_time[j] * quantity[i];
                    }
                }
            }
            if (found == 1)
...
void placeOrder(restaurant rst[], char items[][100], int quantity[], char usr_name[], char phone_no[], address usr_address, int no_of_items, int ordidx[])
{
    ...
    for (int i = 0; i < DB_SIZE && flag == 1; i++)
    {
        if (rst[i].name[0] != '\0')
        {
            delivery_time = isAvailable(rst, items, usr_address.city_name, i, no_of_items,quantity);
            if (delivery_time > 0)
            {
                found = 1;
                flag = 0;
